...
import { INK, BONE, GOLD, SLATE, SLATE_L, MUTED, GREEN, FONT_BODY, FONT_DISPLAY, inp, lbl, errBox, okBox } from './ds'
    ...
    <div style={{ minHeight: '100vh', background: INK, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: FONT_BODY, padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: '400px' }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <h1 style={{ fontFamily: FONT_DISPLAY, fontSize: '24px', color: BONE, margin: '0 0 6px', fontWeight: '700' }}>Create your account</h1>
          <p style={{ color: MUTED, fontSize: '14px', margin: 0 }}>Start using Adsoh</p>
        </div>
        ...
        <div style={{ background: SLATE, border: `1px solid ${SLATE_L}`, borderRadius: '12px', padding: '28px' }}>
          {done ? (
            <div style={okBox}>
              ...
              <p style={{ margin: '6px 0 0', fontSize: '13px' }}>We sent a confirmation link to <strong>{email}</strong>. Click it to activate your account.</p>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <input type="password" value={password} onChange={e => setPassword(e.target.value)} required placeholder="Min. 8 characters" style={{ ...inp, width: '100%', boxSizing: 'border-box' }} />
                </div>
                <div style={{ marginBottom: '20px' }}>
                  <label style={lbl}>Confirm Password</label>
                  <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} required placeholder="••••••••" style={{ ...inp, width: '100%', boxSizing: 'border-box' }} />
                </div>
                <button
                  type="submit"
                  disabled={loading}
